...
pub struct Version {
 ...
 impl Version {
    ...
    pub const fn from_parts(major: u8, minor: u8, patch: u8, data: VersionData) -> Self {
        Self { major, minor, patch, data }
    }
...
mod tests {
    ...
    fn vd(lo: i32, hi: i32, cs: i32) -> VersionData {
        VersionData { lowest_y: lo, highest_y: hi, chunk_size: cs }
    }

    #[test]
    fn display_and_accessors() {
        let v = Version::from_parts(1, 32, 3, vd(-64, 320, 16));
        assert_eq!(v.major(), 1);
    ...
    fn ordering_by_tuple_semantics() {
        let a = Version::from_parts(1, 19, 4, vd(0, 0, 0));
        let b = Version::from_parts(1, 20, 0, vd(0, 0, 0));
        let c = Version::from_parts(1, 20, 1, vd(0, 0, 0));

        assert!(a < b);
        assert!(b < c);
        assert!(a < c);

        let a_same_nums = Version::from_parts(1, 19, 4, vd(-64, 320, 16));
        assert_eq!(a, a_same_nums);
    }

    #[test]
    fn equality_ignores_data_field() {
        let v1 = Version::from_parts(1, 18, 2, vd(-64, 256, 16));
        let v2 = Version::from_parts(1, 18, 2, vd(-128, 1024, 32));
        assert_eq!(v1, v2);

        let v3 = Version::from_parts(1, 18, 3, vd(-64, 256, 16));
        assert_ne!(v1, v3);
    }
    ...
    fn sorting_versions() {
        let mut v = vec![
            Version::from_parts(1, 20, 1, vd(0, 0, 0)),
            Version::from_parts(1, 19, 4, vd(0, 0, 0)),
            Version::from_parts(1, 20, 0, vd(0, 0, 0)),
            Version::from_parts(2, 0, 0, vd(0, 0, 0)),
        ];
        v.sort();
