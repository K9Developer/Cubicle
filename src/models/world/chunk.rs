...
impl<'a> Chunk<'a> {
    pub fn new(version: &'a Version, position: Position, data_version: i32, inhabited_time: Tick, last_update: Tick, status: String) -> Chunk<'a> {
        Chunk {
            version,
    ...
    pub fn entities(&mut self) -> &mut Vec<&'a Entity> { &mut self.entities }
    pub fn structures(&mut self) -> &mut Vec<&'a Structure> { &mut self.structures }
    ...
    pub fn set_status(&mut self, status: &'a str) { self.status = status.to_string(); }
